...
def append_logged_info(sender_address, time):
    ...
    core.add_text(f"Sent Email To {sender_address}", parent="Logs Window", color=rgb)
    core.add_text(f"Took {time} To Execute", parent="Time Window", color=rgb)


def send_server_email(
    sender_address, account_password, subject, body, receiver_address
):
...
def start_main():
    with window("EMail Window"):
        ...
        core.add_button(
            "Send Emails",
            parent="EMail Window",
            callback=lambda: send_server_email(
                f"{core.get_value('Your Email')}",
                f"{core.get_value('Your Email Password')}",
                f"{core.get_value('Email Subject')}",
                f"{core.get_value('Email Body')}",
                f"{core.get_value('Email Reciever')}",
            ),
        )
